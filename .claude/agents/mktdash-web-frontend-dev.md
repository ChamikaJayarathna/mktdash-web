...
Read that table downward when placing new code: if it's a whole screen it's a page; if it's a reusable block of a screen it's a widget; if it's something a user _does_ it's a feature; if it's a domain noun it's an entity; if it carries no business meaning at all it's shared.
...
- **UI system**: shadcn/ui is a **code generator, not a dependency**. There is no `shadcn` package in `package.json` and there must never be one — it was added once and removed deliberately. Pull in a primitive on demand:
  ...
  `dlx` runs the CLI without installing it. It writes real source files into the project per the aliases in `components.json` (`ui` → `@/shared/ui`, `utils` → `@/shared/lib/utils`, style `base-nova`, icon library `lucide`), and that generated code is then ours to edit, review, and commit. Add a component only when a screen actually needs it — never bulk-add the catalogue, and never re-add the CLI as a dependency to "make it easier".

  A freshly generated component is already on-brand: `shared/ui/theme/shadcn-bridge.css` maps shadcn's token names onto ours, so review the output against the token rules rather than restyling it by hand. Primitives come from **Base UI** (`@base-ui/react`), **not Radix** — import from `@base-ui/react/*` (e.g. `@base-ui/react/button`, as `shared/ui/button.tsx` already does); never import `@radix-ui/*`. `cva` for variants, `clsx` + `tailwind-merge` via `cn()` (`shared/lib/utils.ts`), `tw-animate-css` for animation utilities, Lucide React for icons only.
...
- **Application layout**: `react-resizable-panels` for the resizable split views that make up the app's layout — inbox list ↔ conversation, the context panel, canvas side rails. Panel sizes are UI state: persist them in the feature's Zustand store, not the URL.
- **Element resizing**: `re-resizable` for resizing a single element rather than a layout — image and crop boxes in the brand kit, resizable blocks in Template Studio. The rule of thumb: splitting a screen into regions is `react-resizable-panels`; resizing one box is `re-resizable`. Don't use one for the other's job.
- **Media & input widgets**: `react-dropzone` (CSV import drop zone, brand-kit logo upload), `react-easy-crop` (logo/avatar cropping), `react-colorful` (brand-kit color picker).
- **Animation**: CSS only. `tw-animate-css` for Tailwind-level transitions and keyframe utilities, on top of the single easing curve and four named animations defined in `shared/ui/theme/foundations.css` — use those rather than inventing per-component timings. There is deliberately **no JS animation library** in the project. If something genuinely needs timeline orchestration or physics, flag it and get agreement before adding a dependency.
- **Charts**: Recharts (`recharts`), for Reports.
...
- **Authentication**: **not settled — no auth library is currently installed.** `next-auth` was added and then removed deliberately, so don't reintroduce it without confirming. The scaffold still has the `app/api/auth/[...auth]/route.ts` handler slot and `proxy.ts` is specified to read a session and resolve org/workspace scope, so the shape is planned but the library is an open decision: flag it and confirm the approach before writing any sign-in, session, or route-guard code. Whatever is chosen answers _who you are_; CASL (next bullet) answers _what you may do_ — keep the two concerns separate and never use one to substitute for the other.
...
| `semantic.css`      | What a ramp step _means_ here: `--bg-app`, `--bg-rail`, `--bg-panel`, `--fg-heading`, `--fg-body`, `--fg-muted`, `--border-card`, `--border-input`, `--focus-ring`, `--fg-link`. This is the tier components consume.                                     |
...
7. **Tailwind's numeric spacing scale is left at its defaults** (`p-4` is still 16px). The system's odd structural values are _named_ rather than remapped: `gap-row` (9px), `gap-stack` (12px), `p-card` (13px), `p-panel` (16px), `px-gutter` (20px), plus `w-rail`, `w-context-panel`, `h-control`. Raw steps remain reachable as `p-(--space-7)`.
